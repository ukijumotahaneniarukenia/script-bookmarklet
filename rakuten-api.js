...
async function hoge(selectColumnInfoList, downloadFileName) {
  let xpathList = [];
  ...
  await download(resultMergeInfoList, downloadFileName);
      ...
      for (
        ...
        if (selectColumn === "text") {
          ...
          let entry = JSON.parse(
            `{"${selectColumnInfo.name}": ${JSON.stringify(targetText)}}`
          );
...
hoge([
  {
    name: "Title",
    filterRegPtnList: ["\\/div\\[2\\]\\/a\\/div\\[1\\]\\/div$"],
    selectColumnList: ["text"],
    url: "https://api.rakuten.net/collection/free-api",
  },
  {
    name: "Link",
    filterRegPtnList: ["\\/div\\/div\\/div\\[1\\]\\/div\\[2\\]/a$"],
    selectColumnList: ["href"],
    url: "https://api.rakuten.net/collection/free-api",
  },
  {
    name: "Description",
    filterRegPtnList: ["\\/div\\[2\\]\\/a\\/div\\[2\\]\\/div$"],
    selectColumnList: ["text"],
    url: "https://api.rakuten.net/collection/free-api",
  }
], "rakuten-free-api.json");

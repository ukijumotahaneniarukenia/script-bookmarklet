async function hoge(crawlPatternInfoList) {
  ...
  for (
    let crawlPatternIndex = 0;
    crawlPatternIndex < crawlPatternInfoList.length;
    crawlPatternIndex++
  ) {
    const selectColumnInfoList =
      crawlPatternInfoList[crawlPatternIndex].selectColumnInfoList;
    const downloadFileName =
      crawlPatternInfoList[crawlPatternIndex].downloadFileName;

    for (
      let selectColumnInfoIndex = 0;
      selectColumnInfoIndex < selectColumnInfoList.length;
      selectColumnInfoIndex++
    ) {
      ...
      for (
        ...
        patternMatchIndex < selectColumnInfo.patternMatchList.length;
        patternMatchIndex++
      ) {
        const patternMatchInfo =
          selectColumnInfo.patternMatchList[patternMatchIndex];
        ...
        resultInfoList = resultInfoList.concat(
          extractDetailInfo(
            targetXpathList,
            patternMatchInfo.selectColumn,
            selectColumnInfo.name
          )
        );
...
hoge([
  {
    ...
    selectColumnInfoList: [
      ...
      },
      {
        name: "Title",
        patternMatchList: [
          { filterRegPtn: "\\/div\\/h2\\/a\\/span$", selectColumn: "text" },
        ],
      },
      {
        name: "Category",

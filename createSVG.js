...
async function createTargetMainDomText(targetMainDomText, height, width) {
  return new Promise((resolve, reject) => {
    ...
    resolve(targetMainDomText.replace(/TOBE_REPLACE_HEIGHT/, height).replace(/TOBE_REPLACE_WIDTH/, width))
  })
}

async function main(targetMainDomText, height, width, startIndex, endIndex, delta) {
  createTargetDom()
    .then(() => {

function getXpath(targetDom) {
  ...
  if (targetDom && targetDom.parentNode) {
    ...
    for (
      let childIndex = 0;
      childIndex < targetDom.parentNode.childNodes.length;
      childIndex++
    ) {
    ...
    if (1 < sameHierarchyChildList.length) {
      ...
      for (
        let sameHierarchyChildIndex = 0;
        sameHierarchyChildIndex < sameHierarchyChildList.length;
        sameHierarchyChildIndex++
      ) {
...
while (walker.nextNode()) {
  resultInfoList.push({
    ...
    xpath: getXpath(walker.currentNode),
  });
}

function main(isSkipMajorityColor) {
  let resultInfo = {}
  let targetPropertyList = ['background-color', 'color', 'border-top-color', 'border-right-color', 'border-bottom-color', 'border-left-color']
  let skipColorInfo = {
    'rgb(0, 0, 0)': true,
    'rgba(0, 0, 0, 0)': true,
  ...
  for (let targetDomIndex = 0; targetDomIndex < targetDomList.length; targetDomIndex++) {
    const targetDom = targetDomList[targetDomIndex]
    for (let targetPropertyIndex = 0; targetPropertyIndex < targetPropertyList.length; targetPropertyIndex++) {
      const targetProperty = targetPropertyList[targetPropertyIndex]
      let color = window.getComputedStyle(targetDom).getPropertyValue(targetProperty)
      if (!resultInfo[color]) {
        resultInfo[color] = {
          count: 0,
          nodeList: [],
  ...
  function dec2hex(targetDecimalNumber) {
    ...
    return targetHexaDecimalNumber.length == 1 ? '0' + targetHexaDecimalNumber : targetHexaDecimalNumber
  }
...
`
  ...
  console.group(
    outputMessageTemplate(
      ...
      isSkipMajorityColor ? resultInfoList.length - Object.keys(skipColorInfo).length : resultInfoList.length
    )
  )
  resultInfoList.map((resultInfo) => {
    // サブグループ表示開始
    console.groupCollapsed(
      '%c    %c ' + resultInfo.rgbText + ' ' + resultInfo.colorHex + ' %c(' + resultInfo.itemInfo.count + ' times)',
